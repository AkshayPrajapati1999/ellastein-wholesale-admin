...
import { ActiveAndPendingCustomerDetail, UpdatePendingUserDetail } from 'src/graphql/query/customer.query'
...
interface PendingCustomerDetail {
  userId: string
  addressId: number
  storeName: string
  customerCode: string
...
const PendingCustomersDetails = () => {
  ...
  const [formData, setFormData] = useState<PendingCustomerDetail>({
    userId: userId
  } as never as PendingCustomerDetail)
  ...
  useEffect(() => {
    if (data) {
      ...
      const primarySalesChannelNumber = customerDetail.primarySalesChannel
        ? parseInt(customerDetail.primarySalesChannel[0]?.key)
        : 1
      setFormData({ ...customerDetail, primarySalesChannel: primarySalesChannelNumber })
    }
  }, [data])
  ...
  const [updateUser] = useMutation(UpdatePendingUserDetail())
  ...
  const handleUpdatePendingCustomerStatus = (event: any) => {
    ...
    updateUser({
      ...
      .then((data: any) => {
        console.log('Update Pending Customer', data?.updatePendingUserDetail)
  ...
  return (
    <form onSubmit={handleUpdatePendingCustomerStatus}>
      <Grid container spacing={9}>
        ...
        <Grid item xs={12}>
          ...
          <div style={{ display: 'flex', gap: '1rem', marginTop: '1rem' }}>
            ...
            <Link href='/customers/pendingcustomers'>
              <Button size='large' type='submit' variant='contained'>
                Reject
              </Button>
            </Link>
...
export default PendingCustomersDetails
